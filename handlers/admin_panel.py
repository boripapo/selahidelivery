...
async def order_assign(call: CallbackQuery):
    courier_id, order_id = list(map(int, call.data.split("_")[1:]))
    await bot.send_message(call.from_user.id, f"<b>Заказ <code>{order_id}</code> был назначен курьеру {courier_id}</b>")
    await db.update_order_status(order_id, OrderStatus.ASSIGNED, courier_id)
    await bot.send_message(courier_id,
                           get_formatted_order(await db.get_order_by_id(order_id), await db.get_items_by_order_id(order_id)),
